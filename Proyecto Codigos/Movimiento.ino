...
class MaquinaEstado {
  public:
    ...
    void imprimir_estados_historicos() {
      for (int i = 0; i < MAX_HISTORICO; i++) {
        Serial.print(estado_a_texto(estados_historicos[i]));
        Serial.print(" ");
      }
      Serial.println();
    }
...
void loop() {
  ...
  Serial.println(maquina_estado.estado_a_texto(maquina_estado.obtener_estado_actual()));
